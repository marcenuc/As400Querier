...
public class As400QueryTest {
	...
	public static void itShouldInterpolateParameters(final String sourceQuery, final Map<String, String> vars,
			final String expectedParsedQuery) {
		Assert.assertEquals(new As400Query(sourceQuery).getSqlQuery(vars), expectedParsedQuery);
	}
	...
	public static Object[][] predefinedQueries() {
		final File[] queryFiles = new File("queries").listFiles(new FilenameFilterByExtension("sql"));
